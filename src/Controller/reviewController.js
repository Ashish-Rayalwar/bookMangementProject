...
const createReview = async function(req,res){
    ...
    if(!reviewedBy){
        data.reviewedBy = "Guest"
         ...
         if(!validator.isAlpha(reviewedBy.split(" ").join(""))) return res.status(400).send({status:false,message:"plz enter valid name"})
    }
    ...
    let reviewData = {reviewedBy,rating,review,reviewedAt : Date.now()} 
  ...
  const reviewUpdate = async function (req, res) {
    try {
        ...
        if(rating){ if(typeof rating != "number"){ return res.status(400).send({message:"Invalid value of rating"})} }
        if(review){ if(typeof review != "string"){ return res.status(400).send({message:"Invalid value of review"})} }
        if(reviewedBy){ if(typeof reviewedBy != "string"){ return res.status(400).send({message:"Invalid value of reviewedBy"})} }
