...
const registerUser = async (req,res)=>{
    try {
        ...
        if(Object.keys(data).length===0) return res.status(400).send({message:"plz provide valid data"})
        ...
        if(!validTitle.includes(title)) return res.status(400).send({status:false,message:"plz provide valid title"})

        if(!name) return res.status(400).send({status:false,message:"name is mendatory"})
        if(!validator.isAlpha(name.split(" ").join(""))) return res.status(400).send({status:false,message:"plz enter valid name"})
        ...
        if(!validator.isStrongPassword(password)) return res.status(400).send({status:false,message:"plz enter valid password, must contain 1 Uppercase,1 Lowercase,1 special-character"})
        ...
        if(checkDublicateByPhone) return res.status(409).send({status:false,message:"user alredy exist with this phone"})
        if(checkDublicateByEmail) return res.status(409).send({status:false,message:"user alredy exist with this email"})
        let userData = {title,
            name,
            phone,
            email,
...
const loginUser = async(req,res)=>{
   try {
    ...
    if(Object.keys(data).length===0) return res.status(400).send({message:"plz provide valid credentials"})

    let {email,password} = data
  

    if(!email) return res.status(400).send({ status: false, message: "email is missing" })
    if(!validator.isEmail(email.trim())) return res.status(400).send({status:false,message:"Invalid email"})

    if(!password) return res.status(400).send({ status: false, message: "password is missing" })
    if(!validator.isStrongPassword(password)) return res.status(400).send({status:false,message:"Invalid password"})
    ...
    res.status(200).send({status:true,message:"Login scuccess",data:token})
   } catch (error) {
